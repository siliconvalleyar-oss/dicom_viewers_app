...
- Transparent `GestureDetector` overlay with **3 callbacks** (no `onDoubleTap`):
  - `onPanStart` → show floating HUD with current WW/WL values
  - `onPanUpdate` → update values (dy=WL/brightness, dx=WW/contrast) + HUD
  - `onPanEnd` → hide HUD
- **`onDoubleTap`** is on the **mode toggle button** (`InkWell`) instead → **zero delay** on drag start
- HUD uses `AnimatedOpacity` for smooth 200ms fade in/out
- Pinch zoom still works via InteractiveViewer

**✅ Avoid combining `onDoubleTap` + `onPan*` in the same GestureDetector** — it causes ~300ms delay. Put double-tap on a separate button/widget instead.
